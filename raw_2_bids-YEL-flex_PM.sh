...
dcmdir=$toplev/YEL/data/raw/sub-$subj/TARACHAPLIN*
...
peepidcmdir01=${dcmdir}/RFMRI_FIELDMAPSMS-PA_0*
peepirefdcmdir02=${dcmdir}/RFMRI_FIELDMAPSMS-PA_SBREF_0*

# # Location of DICOMS - BOLD: make sure scan numbers match up to correct task!!!
# Resting State
bolddcmdir01=${dcmdir}/*BEN_BOLD*_1_0*
# IAPS
bolddcmdir02=${dcmdir}/RFMRI_BOLD-AP_2_IAPS_1_0*
bolddcmdir03=${dcmdir}/RFMRI_BOLD-AP_3_IAPS_2_0*
# Child Emotion Task
bolddcmdir04=${dcmdir}/RFMRI_BOLD-AP_4_CHILD_EMOTION_TASK1_0*
bolddcmdir05=${dcmdir}/RFMRI_BOLD-AP_5_CHILD_EMOTION_TASK2_0*

# Location of DICOMS - BOLD REF
# Resting State
boldrefdcmdir01=${dcmdir}/*BEN_BOLD*_1_SBREF_0*
# IAPS
boldrefdcmdir02=${dcmdir}/RFMRI_BOLD-AP_2_IAPS_1_SBREF_0*
boldrefdcmdir03=${dcmdir}/*RFMRI_BOLD-AP_3_IAPS_2_SBREF_0*
# Child Emotion Task
boldrefdcmdir04=${dcmdir}/RFMRI_BOLD-AP_4_CHILD_EMOTION_TASK1_SBREF_0*
boldrefdcmdir05=${dcmdir}/RFMRI_BOLD-AP_5_CHILD_EMOTION_TASK2_SBREF_0*
...
if [ -d ${bolddcmdir01} ]; then
    ...
    BOLDfiles=$(ls -1 *BEN_BOLD*_1_0*.nii | wc -l)
    ...
    BOLDfiles=$(ls -1 *BEN_BOLD*_1_0*.json | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
...
if [ -d ${bolddcmdir02} ]; then
    ...
    #Break the func down into each task
    #Capture the number of dissonance files to change
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_2_IAPS_1_0*.nii | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*.nii) #This is to refresh the Checker variable, same as the Anat case
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    tempBOLDext="${tempBOLD##*.}"
    ...
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_2_IAPS_1_0*.json | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*.json) #This is to refresh the Checker variable, same as the Anat case
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    tempBOLDext="${tempBOLD##*.}"
    ...
    #Break the func down into each task
    #Capture the number of dissonance files to change
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_2_IAPS_1_SBREF_0*.nii | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*SBREF*.nii) #This is to refresh the Checker variable, same as the Anat case
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    tempBOLDext="${tempBOLD##*.}"
    ...
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_2_IAPS_1_SBREF_0*.json | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*SBREF*.json) #This is to refresh the Checker variable, same as the Anat case
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    tempBOLDext="${tempBOLD##*.}"
...
if [ -d ${bolddcmdir03} ]; then
    ...
    #Break the func down into each task
    #Capture the number of dissonance files to change
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_3_IAPS_2_0*.nii | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*.nii) #This is to refresh the Checker variable, same as the Anat case
    echo "1"
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    ...
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_3_IAPS_2_0*.json | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*.json) #This is to refresh the Checker variable, same as the Anat case
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    tempBOLDext="${tempBOLD##*.}"
    ...
    #Break the func down into each task
    #Capture the number of dissonance files to change
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_3_IAPS_2_SBREF_0*.nii| wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*SBREF*.nii) #This is to refresh the Checker variable, same as the Anat case
    echo "1"
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    ...
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_3_IAPS_2_SBREF_0*.json | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*SBREF*.json) #This is to refresh the Checker variable, same as the Anat case
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    tempBOLDext="${tempBOLD##*.}"
...
if [ -d ${bolddcmdir04} ]; then
    ...
    #Break the func down into each task
    #Capture the number of dissonance files to change
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_4_CHILD_EMOTION_TASK1_0*.nii | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*.nii) #This is to refresh the Checker variable, same as the Anat case
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    tempBOLDext="${tempBOLD##*.}"
    ...
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_4_CHILD_EMOTION_TASK1_0*.json | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*.json) #This is to refresh the Checker variable, same as the Anat case
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    tempBOLDext="${tempBOLD##*.}"
    ...
    #Break the func down into each task
    #Capture the number of dissonance files to change
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_4_CHILD_EMOTION_TASK1_SBREF_0*.nii| wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*SBREF*.nii) #This is to refresh the Checker variable, same as the Anat case
    echo "1"
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    ...
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_4_CHILD_EMOTION_TASK1_SBREF_0*.json | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*SBREF*.json) #This is to refresh the Checker variable, same as the Anat case
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    tempBOLDext="${tempBOLD##*.}"
...
if [ -d ${bolddcmdir05} ]; then
    ...
    #Break the func down into each task
    #Capture the number of dissonance files to change
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_5_CHILD_EMOTION_TASK2_0*.nii | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*.nii) #This is to refresh the Checker variable, same as the Anat case
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    tempBOLDext="${tempBOLD##*.}"
    ...
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_5_CHILD_EMOTION_TASK2_0*.json | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*.json) #This is to refresh the Checker variable, same as the Anat case
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    tempBOLDext="${tempBOLD##*.}"
    ...
    #Break the func down into each task
    #Capture the number of dissonance files to change
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_5_CHILD_EMOTION_TASK2_SBREF_0*.nii | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*SBREF*.nii) #This is to refresh the Checker variable, same as the Anat case
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    tempBOLDext="${tempBOLD##*.}"
    ...
    BOLDfiles=$(ls -1 *RFMRI_BOLD-AP_5_CHILD_EMOTION_TASK2_SBREF_0*.json | wc -l)
    for ((i=1;i<=BOLDfiles;i++)); do
    BOLD=$(ls *BOLD*SBREF*.json) #This is to refresh the Checker variable, same as the Anat case
    tempBOLD=$(ls -1 $BOLD | sed '1q;d') #Capture new file to change
    tempBOLDext="${tempBOLD##*.}"
